...
describe('persist: computed caching', () => {
  ...
  it('should not restore expired cache and should persist new value on change', () => {
    const base = createTestSignalAdapter()
    const storage = createMemoryAdapter()
    // prewrite expired cached value
    storage.set('cmp3', {value: 10, timestamp: Date.now() - 10_000})
    let src = 1
    const c = persist.computed<number>(base, () => src, {
      persist: true,
      storage,
    ...
    // subscribe to enable persisting
    const unsub = c.subscribe(() => {})
    // change source and recompute → should write fresh value
    src = 2
    c.$recompute()
    const raw = storage.store.get('cmp3')
    expect(raw.value).to.equal(2)
    unsub()
  })

  it('auto-name in dev: computed with persist=true auto-generates name and writes cache', () => {
    const base = createTestSignalAdapter()
    const storage = createMemoryAdapter()
    const src = 5
    const c = persist.computed<number>(base, () => src, {
      persist: true,
      storage,
      // no name provided in dev → auto-name, we cannot know exact name, but cache must appear
    }) as any
    // internal subscription inside persist.computed writes immediately in dev
    expect(storage.store.size).to.equal(1)
    const unsub = c.subscribe(() => {})
    unsub()
  })
})
